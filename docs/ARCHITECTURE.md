...
대상을 향한다:
- **스캔 대상은 확장자로 정해지고, 그 필터는 조용하다(일부 미해결).**
  ...
  - 문제는 이 누락이 **어디에도 드러나지 않는다**는 점이다. 파서에 도달하기
    전에 확장자 필터에서 걸러지므로 `filesUnparsable`에도, `filesAnalysisErrors`
    에도 잡히지 않는다. 그 두 카운터는 "못 봤다"고 리포트에 말하는데(위 참조),
    확장자 필터는 아무 말도 하지 않는다 — 대비되는 지점이다.
  ...
  - **부수 효과: 빌드 계열 파일이 대량으로 수집 대상이 되었다(미해결).** 확장자
    ...
    `build` 역할로 판정할 수 있어야 하는데, `fileRoleClassifier`는 현재
    main/preload/renderer 셋만 판정한다. **`build` 역할 도입 여부는 미해결
    사안**이며, 이 기록은 사실 기록이지 설계 확정이 아니다(결정 #9의 "표본 2건"
    전제는 위 실측으로 대체된다).
  - 남은 **미해결 사안**은 SFC, "확장자 때문에 건너뛴 파일 수"를 세어 노출할지,
    그리고 위 `build` 역할 셋이다. SFC는 `<script>` 블록 추출이 선행되어야 해서
    확장자 하나 더 넣는 문제가 아니라 별도 기능이다.
