...
대상을 향한다:
- **스캔 대상은 확장자로 정해진다. 그 필터의 침묵은 닫았고, SFC 추출은 측정 후 보류했다.**
  `scanner.ts`가 소스로 수집하는 확장자는
  `.js`/`.ts`/`.jsx`/`.tsx`/`.mjs`/`.cjs`/`.mts`/`.cts` 여덟이고, `.html`/`.htm`
  은 `<meta>` CSP 추출 용도로만 따로 읽는다. 그 밖의 확장자는 — `.vue`·`.svelte`
  같은 SFC(single-file component) — **수집 자체가 되지 않는다.**
  - 이 누락은 오랫동안 **어디에도 드러나지 않았다**. 파서에 도달하기 전에
    확장자 필터에서 걸러지므로 `filesUnparsable`에도, `filesAnalysisErrors`에도
    잡히지 않았다 — 그 두 카운터는 "못 봤다"고 말하는데 확장자 필터만 침묵했다.
    **이제 `.vue`/`.svelte`는 `ScanResult.skippedUnsupported`로 센다.** 크기 상한
    초과 파일처럼 **열지 않고 세기만** 하며, `ReportMeta.filesSkippedUnsupported`
    → 터미널 skip-note·Markdown scan-note·JSON `summary`(항상 숫자, 0 포함)로
    나간다. SARIF에는 넣지 않았다 — SARIF는 다른 스캔 카운터도 하나도 싣지 않아
    이것만 넣으면 오히려 비일관이다. 문구는 "건너뛴 파일이 있다"가 아니라
    **무엇이 분석되지 않았는가**를 말한다("if your renderer lives there, it is
    outside this scan" — 단정이 아니라 조건문인 이유는 그 파일이 렌더러라는
    보장이 없기 때문이다). 세 카운터(파싱 실패·분석 실패·애초에 안 읽음)는
    실패 단계가 다르므로 **별도로 유지**한다 — 합치면 "왜 이 파일이 빠졌는가"를
    사람이 추적할 수 없다. 대상은 `.vue`/`.svelte` 둘만이다: 미지원 확장자 전부를
    세면 한 프로젝트에서 4,528건(대부분 png·json·md), clean 코퍼스에서도 66건이
    나와 신호로 무의미하고, `.astro`·`.coffee` 같은 다른 컴포넌트 포맷은 어느
    코퍼스에도 없어 넣으면 추측이 된다. 실전 검증: 재스캔에서 `.svelte` 파일
    수와 카운터가 4/4 일치했고(473/37/40/94), Vue 6개 앱도 일치했다.
  - **결과**: 렌더러를 SFC로 작성한 프로젝트는 렌더러가 통째로 분석 밖에 있는데
    리포트는 정상 완료로 보인다. 실측에서 Svelte 기반 프로젝트를 스캔했을 때
    수집 파일이 10개에 그친 관찰이 있다 — 실패가 아니라 "볼 대상이 그것뿐"으로
    보고된다.
  - **`.mjs`/`.cjs`/`.mts`/`.cts`는 이 목록에 추가해 닫았다.** 이중 모듈
    체계만 다를 뿐 이미 다루던 그 JS/TS이고, 실제 Electron 프로젝트의 빌드·
    패키징·코드서명 스크립트가 그 확장자에 산다 — 셸 명령을 경로 보간으로
    조립하는 코드가 가장 흔한 자리다. 규칙은 하나도 손대지 않았고
    `fileRoleClassifier`도 확장자를 가정하지 않으므로 그대로다. 파서 쪽
    `TS_NO_JSX_EXTENSIONS`의 `.mts`/`.cts` 항목은 그동안 스캐너가 그 확장자를
    넘겨준 적이 없어 **도달 불가능한 분기**였는데, 이제 실제로 쓰인다.
  - **부수 효과: 빌드 계열 파일이 대량으로 수집 대상이 되었다(미해결).** 확장자
    확대로 새로 수집되는 파일 174개를 분류해 보면 빌드/스크립트 88 · 설정 11 ·
    eslint류 5로, 절반 이상이 런타임이 아니라 빌드 시점 코드다. 실제 서드파티
    앱을 스캔했을 때 나온 신규 finding 5건 중 **3건이 코드 서명 빌드 스크립트의
    `execSync` 보간**(EA020 critical/high)이었다. 이 발화는 **규칙상 정확하다** —
    보간된 경로로 셸 명령을 조립하는 것은 실제 위험이고, 규칙은 그것을 그대로
    말한다. 다만 빌드 스크립트는 런타임 코드와 **위험의 성격이 다르다**: 공격
    시점이 앱 실행 중이 아니라 공급망(빌드·배포) 쪽이고, 입력을 쥐는 주체도
    최종 사용자가 아니라 빌드 환경이다. 이 차이를 리포트에 반영하려면 파일을
    `build` 역할로 판정할 수 있어야 한다. **`build` 역할은 0.1.8에서 도입해
    닫았다**(위 엔진 계약의 파일 역할 분류 참조). 이 단락은 그 결정의 근거가 된
    실측 기록으로 남긴다(결정 #9의 "표본 2건" 전제는 위 실측으로 대체됐다).
  - 이 절에 미해결로 남겨 두었던 셋 — `build` 역할, 건너뛴 파일 수 노출, SFC —
    은 모두 결론이 났다. 앞의 둘은 위와 같이 구현했고, **SFC(`.vue`/`.svelte`)의
    `<script>` 추출은 측정 후 보류**했다. 보존 코퍼스의 `.vue` 758개(6개
    프로젝트, Vue 2·3 혼재)에서 블록 742개를 추출해 전 규칙을 돌린 결과 finding은
    9건, 전부 EA040 high/heuristic이고 high-confidence는 0건이었다 — 렌더러
    컴포넌트에는 이 도구의 규칙이 보는 싱크가 거의 없다. 정규식 추출 자체는
    성립하고(`<script\b[^>]*>[\s\S]*?<\/script>`의 선형 패턴으로 742/742 파싱,
    줄 번호는 블록 시작 줄을 기록해 더하는 오프셋 보정으로 9/9 정확) 컴파일러
    의존성(`@vue/compiler-sfc`·`svelte`, 합쳐 런타임 의존성 24개·5MB)도 불필요하지만,
    scanner·parser 두 입구 계층을 넓히는 대가가 회수와 맞지 않아 넣지 않았다 —
    데코레이터 건(23파일 복구, finding 0)과 같은 결론이다. `.svelte`도 이후
    재스캔에서 측정했다: Svelte 4·5 앱 4개의 `.svelte` 644개(473+37+40+94)에서
    `<script>` 블록 187개를 추출했고(파일당 0~2, 대부분이 `<script>` 없는 SVG
    아이콘 컴포넌트), 전 규칙 실행 결과 **finding 0건, high-confidence 0건**으로
    Vue(758파일 → heuristic 9건)보다 조용했다. 부정 결과는 교차 확인했다 —
    187블록 어디에도 `electron`·`ipcRenderer`·`openExternal`·`child_process` 토큰이
    없고, 렌더러는 preload가 심은 `window.*` 전역으로만 main에 닿는다. 태그 형태는
    3종이다: `lang="ts"` 160 · 속성 없음 21 · `lang="ts" module` 6(Svelte 5 문법,
    Vue에 없던 형태). **SFC 보류는 Vue·Svelte 양쪽 실측에 근거한다.** 재검토 시를
    위해 role 판단도 적어 둔다:
    `.vue`/`.svelte` 확장자를 renderer의 증거로 쓰는 것은 정당하다. 템플릿을 가진
    UI 컴포넌트 포맷이라 Electron main 프로세스에서 렌더할 수 없으므로 디렉토리
    이름 같은 **관례가 아니라 파일 포맷이라는 사실**이고, B2가 거부한 "일치하는 게
    없으니 renderer" 폴백과는 다르다. 지금은 추출을 하지 않으므로 필요 없다.
...
- 대상 코드 처리에 정규식을 쓸 때는 백트래킹 폭발(ReDoS)이 없는 선형 패턴만
  사용한다.
- **재스캔에서 발견됐으나 아직 손대지 않은 셋(재론 방지용 기록 — 사실만 적고
  해법은 확정하지 않는다).**
  - **(a) 모노레포 루트의 G그룹 침묵.** 루트 `package.json`에 electron 의존이
    없으면(workspaces만 있는 경우) `ProjectContext.electronMajorVersion`·
    `dependencyNames`가 비어 EA060/061/062가 통째로 침묵한다. 하위 패키지를
    스캔하면 나온다. 0.1.9의 nested manifest 탐색은 role 판정에만 적용됐고
    `ProjectContext`에는 적용되지 않았다 — `scanner.ts`는 지금도 스캔 루트의
    manifest 하나만 읽는다. 재스캔에서 루트 manifest에 electron 의존이 없는
    모노레포가 3개 관찰됐고, 그중 2개에서 같은 트리를 하위 패키지 루트로 스캔하면
    EA062가 나오는 것을 확인했다. **사용자가 알 방법이 없는 미탐**이라 다음 조사
    대상이다.
  - **(b) 접두어 결합 채널.** `ipcRenderer.invoke(prefix + "." + channel, ...)`처럼
    호출자가 채널의 일부(접미어)를 고르는 형태는 EA031의 "그대로 전달" 판정에
    걸리지 않는다. 위험은 같은 종류이나 `prefix`가 고정이라 채널 공간이 좁다.
    판정 확장 여부는 미결이다.
  - **(c) `filesAnalysisErrors`가 파일명을 노출하지 않는다.** 카운터는 리포트에
    나오지만 어느 파일인지는 `ELECTRON_AUDIT_DEBUG=1`로만 보인다. 카운터를
    노출하면서 위치를 숨기는 건 반쪽이다 — "왜 하나 덜 스캔됐는지"를 사용자가
    추적할 수 없다.
