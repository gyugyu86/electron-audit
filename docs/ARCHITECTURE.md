...
대상을 향한다:
- **스캔 대상은 확장자로 정해지고, 그 필터는 조용하다(미해결).** `scanner.ts`가
  소스로 수집하는 확장자는 `.js`/`.ts`/`.jsx`/`.tsx` 넷뿐이고, `.html`/`.htm`은
  `<meta>` CSP 추출 용도로만 따로 읽는다. 그 밖의 확장자는 — `.vue`·`.svelte`
  같은 SFC(single-file component)는 물론 `.mjs`/`.cjs`/`.mts`/`.cts`까지 —
  **수집 자체가 되지 않는다.**
  ...
  - **결과**: 렌더러를 SFC로 작성한 프로젝트는 렌더러가 통째로 분석 밖에 있는데
    ...
    보고된다.
  - 이건 현재 상태의 사실 기록이지 해법 확정이 아니다. **미해결 사안**으로,
    최소한 "확장자 때문에 건너뛴 파일 수"를 세어 노출할지, SFC의 `<script>`
    블록을 추출할지는 아직 정하지 않았다.
