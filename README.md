...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: gyugyu86/electron-audit@v0.1.7
        with:
          path: .                  # project to scan
          version: 0.1.7           # pin the scanner (default: latest)
          # fail-on-findings: true # fail on high-confidence critical/high (default)
          # upload: true           # upload to code scanning (default)
```

`@v0.1.7` is the recommended pin. For the strongest supply-chain guarantee, pin
the commit SHA the tag resolves to instead — an author can move a tag, but not a
commit SHA. Run `git rev-parse v0.1.7^{commit}` and use
`uses: gyugyu86/electron-audit@<sha>  # v0.1.7`.
